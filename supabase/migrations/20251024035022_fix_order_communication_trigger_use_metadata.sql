/*
  # Fix trigger de comunicación - usar metadata en lugar de dogcatify_data

  1. Changes
    - Actualiza trigger_send_order_communication()
    - Usa el campo 'metadata' en lugar de 'dogcatify_data'
    - El campo metadata ya existe en la tabla orders

  2. Notes
    - Corrige el error "record has no field dogcatify_data"
    - Mantiene la misma lógica de construcción del payload
*/
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
    ...
    -- Obtener metadata de la orden (campo que existe en la tabla)
    v_order_metadata := COALESCE(NEW.metadata, '{}'::jsonb);

    -- Construir payload para la comunicación con los datos de metadata
    v_payload := jsonb_build_object(
      ...
      'data', jsonb_build_object(
        'client_name', COALESCE(
          v_order_metadata->>'client_name',
          v_client_record.contact_name,
          v_client_record.company_name
        ),
        'service_name', COALESCE(v_order_metadata->>'service_name', 'Servicio'),
        'provider_name', COALESCE(v_order_metadata->>'provider_name', 'Proveedor'),
        'reservation_date', COALESCE(v_order_metadata->>'reservation_date', NEW.order_date::text),
        'reservation_time', COALESCE(v_order_metadata->>'reservation_time', ''),
        'pet_name', COALESCE(v_order_metadata->>'pet_name', '')
      )
    );

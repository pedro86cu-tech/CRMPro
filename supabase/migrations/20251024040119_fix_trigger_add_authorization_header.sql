/*
  # Fix: Agregar header de autorización en trigger

  1. Changes
    - Actualiza trigger_send_order_communication() para incluir Authorization header
    - Usa el service_role_key para autenticar la llamada
    - La clave está disponible como variable de entorno en Supabase

  2. Notes
    - service_role_key: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNhdHprcHlubnVsb25jd2d4ZWV2Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MDE1MjIyMSwiZXhwIjoyMDc1NzI4MjIxfQ.ky5BshFJzJ3JXSW6vkhxXQGaJ3t8T9bK7lqyFAXuqr4
    - Esta clave permite llamar a edge functions con permisos elevados
*/
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
    ...
    RAISE NOTICE 'Payload construido para orden %', NEW.id;

    -- Llamar a la edge function usando pg_net con autorización
    BEGIN
      SELECT net.http_post(
        url := 'https://satzkpynnuloncwgxeev.supabase.co/functions/v1/send-order-communication',
        headers := jsonb_build_object(
          'Content-Type', 'application/json',
          'Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNhdHprcHlubnVsb25jd2d4ZWV2Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MDE1MjIyMSwiZXhwIjoyMDc1NzI4MjIxfQ.ky5BshFJzJ3JXSW6vkhxXQGaJ3t8T9bK7lqyFAXuqr4'
        ),
        body := v_payload
      ) INTO v_request_id;
      
      RAISE NOTICE 'Llamada HTTP iniciada con request_id: % para orden %', v_request_id, NEW.id;
      
    EXCEPTION
      WHEN OTHERS THEN
        -- Si falla pg_net, registrar error

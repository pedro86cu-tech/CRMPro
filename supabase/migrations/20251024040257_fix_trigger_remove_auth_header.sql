/*
  # Fix: Trigger sin header de autorización

  1. Changes
    - Actualiza trigger_send_order_communication() para NO incluir Authorization header
    - La edge function ahora tiene verify_jwt: false
    - Simplifica la llamada HTTP

  2. Notes
    - Edge function configurada con verify_jwt: false
    - No requiere autenticación
    - pg_net hace la llamada asíncrona
*/
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
    ...
    -- Llamar a la edge function usando pg_net (sin autorización, verify_jwt=false)
    BEGIN

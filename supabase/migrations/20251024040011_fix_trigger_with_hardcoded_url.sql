/*
  # Fix: Trigger con URL hardcodeada para llamar a edge function

  1. Changes
    - Actualiza trigger_send_order_communication() con URL de Supabase hardcodeada
    - Usa pg_net.http_post() directamente con la URL completa
    - Construye el payload correctamente desde metadata

  2. Notes
    - La URL de Supabase es: https://satzkpynnuloncwgxeev.supabase.co
    - pg_net hace llamadas HTTP asíncronas
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        RAISE NOTICE 'Error con pg_net (%)', SQLERRM;
    END;
